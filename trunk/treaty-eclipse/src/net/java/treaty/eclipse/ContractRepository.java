...
/**
 * Contract repository. This is a singleton. This class manages all contracts
...
public class ContractRepository {
		...
		for (EclipsePlugin plugin : getPluginsWithContracts()) {
			for (EclipseExtensionPoint xp : plugin.getExtensionPoints()) {
				for (EclipseExtension x : xp.getExtensions()) {
					...
					for (Contract contract : x.getContracts()) {
	...
	public static void reset(IJobChangeListener listener) {
		...
		final ContractLoadingJob job =
				new ContractLoadingJob("Reloading contracts", false);
	...
	public static void init(IJobChangeListener listener) {
		...
		final ContractLoadingJob job =
				new ContractLoadingJob("Loading contracts", true);
		job.addJobChangeListener(listener);
		job.schedule();
	}
	...
	public void verify(Collection<Contract> contracts,
			VerificationReport verReport, VerificationJobListener vListener,
			IJobChangeListener jListener) {
