...
public class Tests {
	...
	public void setUp() throws Exception {
		...
		IExtension x = registry.getExtension("net.java.treaty.eclipse.vocabulary.junit");
	...
	private Resource getResource(String type,String clazz) throws Exception {
		...
		Object value = new EclipseVerifier().load(r.getType(),r.getName(), connector);
		r.setValue(value);
		return r;
	}
	...
	public void testTypes1() throws Exception {
		Resource r = this.getResource(VOC.TESTCASE, DateFormatterFunctionalTests.class.getName());
		ExistsCondition c = new ExistsCondition();
		c.setResource(r);
		VOC.check(c);
	}
	@Test(expected = VerificationException.class) // does not have the right constructors
	public void testTypes2() throws Exception {
		Resource r = this.getResource(VOC.TESTCASE, Tests.class.getName());
		ExistsCondition c = new ExistsCondition();
		c.setResource(r);
		VOC.check(c);
	}
	...
	public void testTypes3() throws Exception {
		Resource r = this.getResource(VOC.TESTCASE, EmptyTestCase.class.getName());
		ExistsCondition c = new ExistsCondition();
		c.setResource(r);
		VOC.check(c);
	}
	...
	public void testRelationship1() throws Exception {
		Resource r1 = this.getResource(VOC.TESTCASE, DateFormatterFunctionalTests.class.getName());
		Resource r2 = this.getResource(VOC.INSTANTIABLE_CLASS, ShortDateFormatter.class.getName());
		RelationshipCondition rel = new RelationshipCondition();
		rel.setResource1(r2);
		rel.setResource2(r1);
		rel.setRelationship(new URI(VOC.VERIFIES));
		VOC.check(rel);
	}
	@Test(expected = VerificationException.class)
	public void testRelationship2() throws Exception {
		Resource r1 = this.getResource(VOC.TESTCASE, DateFormatterFunctionalTests.class.getName());
		Resource r2 = this.getResource(VOC.INSTANTIABLE_CLASS, TooShortDateFormatter.class.getName());
		RelationshipCondition rel = new RelationshipCondition();
		rel.setResource1(r2);
		rel.setResource2(r1);
		rel.setRelationship(new URI(VOC.VERIFIES));
