...
public class SimpleContract implements ConditionContext, Visitable, Contract {
	...
	public SimpleContract(Connector consumer,Connector connector2,String contractLocation) {
		...
		assert(consumer.getRole()==Role.COSUMER);
		this.consumer = consumer;
		assert(consumer.getRole()==Role.SUPPLIER);
	...
	public void loadResources(Component component,ResourceLoader loader) throws InvalidContractException {
		for (Resource r:this.consumerResources.values())
			r.resolve(component,loader);
		for (Resource r:this.supplierResources.values())
			r.resolve(component,loader);
	}
	/* (non-Javadoc)
	 ...
	 */
	public Contract bindSupplier(Connector connector,ResourceLoader loader) throws InvalidContractException {
	 ...
	 */
	public Contract bindConsumer(Connector connector,ResourceLoader loader) throws InvalidContractException {
		SimpleContract contract = new SimpleContract();
		contract.setLocation(this.getLocation());
